...
const autoFile = () => join(agentDir, 'ralph', 'test-session.db');
/** Read a ralph file from disk (a SQLite database; legacy text auto-migrates). */
const readBacklog = (path: string = autoFile()): Backlog => Backlog.open(path);
/** The rendered text form of a ralph file (for snapshot comparisons). */
const renderFile = (path: string = autoFile()): string => Backlog.open(path).render();
...
describe('ralph-loop extension', () => {
	...
	test('loop-police stream abort nudges instead of pausing: the recovery turn runs, and the fallback queues the cycle if the model does not comply', async () => {
		...
		// the run itself. No nudge steer yet — a steer now would join the
		// dying run and be dropped (pi ends the run on an aborted stop without
		// draining the steering queue); the settle delivers it with the
		// recovery turn it starts.
		...
		// settle starts the recovery turn with the nudge delivered.
		await fake.fire('message_end', fakeCtx.ctx, { message: { role: 'assistant', stopReason: 'aborted' } });
		await fake.fire('agent_settled', fakeCtx.ctx);
		expect(statusLine(fakeCtx.widgets)).not.toContain('paused');
		expect(statusLine(fakeCtx.widgets)).not.toContain('finishing');
		expect(fake.userMessages.length).toBe(2);
		expect(fake.userMessages[1].text).toContain('A reasoning loop was detected');
		expect(fake.userMessages[1].options).toEqual({ deliverAs: 'followUp' });
	...
	test('loop-police output-loop abort does not pause the loop either (no nudge, recovery turn continues the work)', async () => {
		const fake = createFakePi();
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		...
		// Output loops are not reasoning events: ralph sends no nudge, but the
		...
		await fake.fire('agent_settled', fakeCtx.ctx);
		expect(statusLine(fakeCtx.widgets)).not.toContain('paused');
		// The settle starts the recovery turn (loop-police's own triggerTurn
		// message was lost with the abort): a plain continue prompt, no nudge.
		expect(fake.userMessages.length).toBe(2);
		expect(fake.userMessages[1].text).toContain('cut by loop-police');
		expect(fake.userMessages[1].options).toEqual({ deliverAs: 'followUp' });
	...
	test('completed-task cycle increments the iteration and resets the per-task counter', async () => {
		...
		await writeFile(autoFile(), RALPH_V2_TASK_ONE_DONE);
		fakeCtx.usagePercent.value = 10;
		await fake.fire('agent_settled', fakeCtx.ctx);
...
describe('ralph-loop extension (SQLite-backed ralph format)', () => {
	...
	test('/ralph import converts a Markdown backlog and tracks its source', async () => {
		...
		expect(imported.sources()).toEqual(['TODO.md']);
		expect(imported.listTasks().find((t) => t.title === 'Establish a clean local developer contract.')?.category).toBe('General');
		expect(imported.listTasks().find((t) => t.title === 'Add sign-in.')?.done).toBe(true);
		expect(imported.counts()).toEqual({ open: 2, total: 3, completed: 1 });
		expect(imported.listLogEntries().map((e) => [e.taskId, e.date])).toEqual([[3, '2026-08-10']]);
...
`;
		...
		const merged = readBacklog();
		expect(merged.sources()).toEqual(['TODO.md', 'FEATURES.md']);
	...
	test('/ralph import always targets the session ralph file and prompts only for the category', async () => {
		const fake = createFakePi();
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		await fake.fire('session_start', fakeCtx.ctx, { reason: 'startup' });
		...
		// Second import: merges into the same session file; --category skips the prompt.
	...
	test('start with a ralph-format backlog uses the ralph_todo prompt and tool', async () => {
		...
		expect(fake.userMessages[0].text).toContain('The backlog is accessible with the ralph_todo tool.');
	...
	test('status counter marks the current task done when no open tasks remain', async () => {
		...
		for (const task of ['1', '2']) {
			...
			fakeCtx.usagePercent.value = 10;
			if (task === '2') {
				// As soon as the last task completes the counter says done
				// instead of a misleading 3/3; the loop winds down (recording
				// the completion, then stopping on the finished backlog) at
				// the next settle.
		...
		// The finished backlog stops the loop once the recording turn settles.
		expect(statusLine(fakeCtx.widgets)).toContain('Ralph: off');
	});

	test('start --category scopes the backlog; a new category is created but must hold tasks', async () => {
		const fake = createFakePi();
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		await fake.fire('session_start', fakeCtx.ctx, { reason: 'startup' });
		...
		// Give the tasks categories by editing the imported backlog directly.
		const withCategories = readBacklog();
		withCategories.updateTaskById(1, { category: 'dossier' });
		withCategories.updateTaskById(2, { category: 'dossier' });
		withCategories.updateTaskById(3, { category: 'auth' });
		withCategories.save(autoFile());

		const ralph = fake.commands.get('ralph')!;
		fakeCtx.notifications.length = 0;
		await ralph.handler('start --category nope', fakeCtx.ctx);
		expect(fake.userMessages.length).toBe(0);
		expect(fakeCtx.notifications.at(-1)?.message).toContain('category "nope" has no tasks');
		// The refused start leaves the session file untouched.
		expect(readBacklog().createdLists()).not.toContain('nope');

		fakeCtx.notifications.length = 0;
		await ralph.handler('start --category dossier', fakeCtx.ctx);
		expect(fake.userMessages.length).toBe(1);
	...
	test('ralph_todo tool completes, checkpoints, logs, and adds tasks in the file', async () => {
		...
		expect(file().listTasks()[0]?.done).toBe(true);
		...
		const afterCheckpoint = file();
		expect(afterCheckpoint.listTasks()[1]?.checkpoint).toBe('Removed routes; next step is the shell.');
		expect(afterCheckpoint.listTasks()[1]?.checkpointIteration).toBe(1);
		...
		expect(file().listLogEntriesForTask(1).map((e) => [e.date, e.note])).toEqual([['2026-08-11', 'Replaced the README. Verified: bun test.']]);
		...
		expect(file().listTasks().find((t) => t.title === 'Add sign-out.')?.category).toBe('auth');
		...
		// The file stays a valid ralph backlog after every mutation.
		expect(file().listTasks()).toHaveLength(4);
	});
	...
	test('ralph_todo complete with a note records the completion log entry in the same call', async () => {
		...
		expect(file().listTasks()[0]?.done).toBe(true);
		expect(file().listLogEntriesForTask(1).map((e) => e.note)).toEqual(['Replaced the README. Verified: bun test.']);
	...
	test('ralph_todo manages the session ralph file from chat without an active loop', async () => {
		const fake = createFakePi();
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		await fake.fire('session_start', fakeCtx.ctx, { reason: 'startup' });
		...
		// Adding to a list that does not exist yet creates it (session backlog).
		const addedMissing = await run({ action: 'add', title: 'Nope.', category: 'Missing' });
		expect(addedMissing.content[0]!.text).toContain('category "Missing"');
		// Adding without a category defaults to the session category.
		const addedDefault = await run({ action: 'add', title: 'No list.' });
		expect(addedDefault.content[0]!.text).toContain('category "General"');
	...
	test('ralph_todo add-many adds a batch of tasks all-or-nothing', async () => {
		...
		expect(readBacklog().listTasks().length).toBe(before);

		// An unknown batch category is created (session backlog): the whole batch lands.
		const addedNope = await run({ action: 'add-many', category: 'Nope', tasks: [{ title: 'X.' }, { title: 'Y.', category: 'Docs' }] });
		expect(addedNope.content[0]!.text).toContain('Added 2 tasks:');
		expect(readBacklog().categories()).toContain('Nope');

		// An unknown per-entry override is created as well.
		const addedOverride = await run({ action: 'add-many', category: 'Infra', tasks: [{ title: 'X.' }, { title: 'Y.', category: 'Nope' }] });
		expect(addedOverride.content[0]!.text).toContain('Added 2 tasks:');
	...
	test('ralph_todo search finds tasks by keyword without writing the file', async () => {
		...
		expect(renderFile()).toBe(before);
	});


	test('ralph_todo import converts a Markdown backlog into the session ralph file', async () => {
		const fake = createFakePi();
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		await fake.fire('session_start', fakeCtx.ctx, { reason: 'startup' });
		...
		// Import without a loop creates the session ralph file from the Markdown file.
		...
		let file = readBacklog();
		expect(file.sources()).toEqual(['TODO.md']);
		expect(file.listTasks()[0]?.title).toBe('Establish a clean local developer contract.');
		expect(file.listTasks()[0]?.category).toBe('General');

		// Re-importing the same source is refused (the source is recorded).
		await expect(run({ action: 'import', file: 'TODO.md' })).rejects.toThrow(/already imported into/);
		...
		file = readBacklog();
		expect(file.sources()).toEqual(['TODO.md', 'EMAIL.md']);
		expect(file.listTasks().find((t) => t.title === 'Draft the reply template.')?.category).toBe('Email');

		// Non-Markdown input, missing files, and a missing path are refused.
		await expect(run({ action: 'import', file: 'TODO.ralph' })).rejects.toThrow(/only accepts Markdown TODO files/);
	...
	test('ralph_todo without a loop needs a backlog; init bootstraps it; a Markdown TODO is refused at start', async () => {
		...
		// A non-ralph session file is refused at start: import or replace it first.
		const ralph = fake.commands.get('ralph')!;
		await writeFile(autoFile(), '- [ ] a markdown task\n');
		await ralph.handler('start', fakeCtx.ctx);
		expect(fakeCtx.notifications.at(-1)?.message).toContain('Ralph loops run on ralph-format backlogs only');
	});
	...
	test('ralph_todo init bootstraps a missing backlog and is idempotent', async () => {
		...
		const created = await run({ action: 'init' });
		expect(created.content[0]!.text).toContain('Created empty Ralph backlog');
		expect(readBacklog().listTasks()).toHaveLength(0);
		...
		await writeFile(autoFile(), '# not a backlog\n');
...
`;
	...
	test('start --goal is refused when the backlog has no goal', async () => {
		...
		await fake.fire('session_start', fakeCtx.ctx, { reason: 'startup' });
		const ralph = fake.commands.get('ralph')!;
		await ralph.handler('start --goal', fakeCtx.ctx);
		expect(fake.userMessages.length).toBe(0);
	...
	test('start --goal is refused when the goal is already complete', async () => {
		...
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		await writeFile(autoFile(), GOAL_DONE);
		await fake.fire('session_start', fakeCtx.ctx, { reason: 'startup' });
		const ralph = fake.commands.get('ralph')!;
		await ralph.handler('start --goal', fakeCtx.ctx);
		expect(fake.userMessages.length).toBe(0);
	...
	test('start --goal starts with an open goal and zero open tasks', async () => {
		...
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		await writeFile(autoFile(), GOAL_NO_TASKS);
		await fake.fire('session_start', fakeCtx.ctx, { reason: 'startup' });
		const ralph = fake.commands.get('ralph')!;
		await ralph.handler('start --goal', fakeCtx.ctx);
		expect(fake.userMessages.length).toBe(1);
	...
	test('start --goal starts with an open goal and open tasks', async () => {
		...
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		await writeFile(autoFile(), GOAL_OPEN);
		await fake.fire('session_start', fakeCtx.ctx, { reason: 'startup' });
		const ralph = fake.commands.get('ralph')!;
		await ralph.handler('start --goal', fakeCtx.ctx);
		expect(fake.userMessages.length).toBe(1);
	...
	test('start without --goal is unchanged: finished backlog refused, goal ignored', async () => {
		...
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		await writeFile(autoFile(), RALPH_FINISHED);
		await fake.fire('session_start', fakeCtx.ctx, { reason: 'startup' });
		const ralph = fake.commands.get('ralph')!;
...
`;
	...
	async function startGoalLoop(content: string, goal = true) {
		...
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		await writeFile(autoFile(), content);
		await fake.fire('session_start', fakeCtx.ctx, { reason: 'startup' });
		await fake.commands.get('ralph')!.handler(`start${goal ? ' --goal' : ''}`, fakeCtx.ctx);
		return { fake, fakeCtx };
	}
	...
	test('planning prompt: goal open, zero tasks', async () => {
		...
		expect(prompt).toContain('The goal (status: open):');
...
`;
	...
	async function startLoopWith(content: string, mode: 'goal' | 'tasks' = 'goal') {
		...
		await writeFile(autoFile(), content);
		await fake.fire('session_start', fakeCtx.ctx, { reason: 'startup' });
		await fake.commands.get('ralph')!.handler(`start${mode === 'goal' ? ' --goal' : ''}`, fakeCtx.ctx);
	...
	test('show works outside a loop on TODO.ralph', async () => {
		...
		expect(shown.content[0]!.text).toContain('Goal (status: open)');
	...
	test('show reports a missing goal, a missing file, and a non-ralph file', async () => {
		...
		await writeFile(autoFile(), '# Plain\n\n- [ ] task\n');
		await expect(run({ action: 'show' })).rejects.toThrow(/not a ralph-format backlog/);
	});

	test('show targets the active loop’s backlog', async () => {
	...
	test('checkpoint replaces the single goal checkpoint in the file', async () => {
		...
		expect(first.content[0]!.text).toContain('Checkpoint recorded for the goal (iteration 1)');
		let goal = file().goal()!;
		expect(goal.checkpoint).toBe('Plan drafted; next: review the routes.');
	...
	test('complete requires an active goal loop, an open goal, no open tasks, and evidence', async () => {
		...
		expect(readBacklog().goal()?.status).toBe('open');
	...
	test('complete goes straight to done when auto-approve decisions is enabled', async () => {
		...
		await writeFile(autoFile(), GOAL_OPEN_TASKS_DONE);
		await fake.fire('session_start', fakeCtx.ctx, { reason: 'startup' });
		await fake.commands.get('ralph')!.handler('start --goal', fakeCtx.ctx);
		const run = (params: Record<string, unknown>) => goalTool(fake).execute('t', params, undefined, undefined, fakeCtx.ctx);
		...
		expect(result.content[0]!.text).toContain('The goal is done (approver: auto-approved)');
	...
	test('confirm requires an active goal loop and a claimed goal', async () => {
		...
		const fakeCtx = createFakeCtx(dir);
		await fake.fire('session_start', fakeCtx.ctx, { reason: 'startup' });
		await writeFile(autoFile(), GOAL_CLAIMED);
		const run = (params: Record<string, unknown>) => goalTool(fake).execute('t', params, undefined, undefined, fakeCtx.ctx);
	...
	test('confirm marks a claimed goal done, keeping its evidence', async () => {
		...
		expect(result.terminate).toBeUndefined();
		expect(result.content[0]!.text).toContain('The goal is done (approved)');
...
`;
	...
	test('agent_settled cycles a planning iteration that adds tasks into an execution iteration', async () => {
		...
		await writeFile(autoFile(), GOAL_EXECUTION);
		fakeCtx.usagePercent.value = 10;
		await fake.fire('agent_settled', fakeCtx.ctx);
		await flush();
	...
	test('agent_settled does not cycle when tasks are edited without changing the open set', async () => {
		...
		await writeFile(autoFile(), edited);
		fakeCtx.usagePercent.value = 10;
		await fake.fire('agent_settled', fakeCtx.ctx);
		await flush();
...
describe('ralph-loop extension (/ralph home view)', () => {
	...
	test('bare /ralph opens the home view for the active loop; enter opens the task view, q closes it', async () => {
		...
		expect(home).toContain('Ralph home — test-session.db');
		...
		expect(rendered).toContain('Ralph backlog — test-session.db');
	...
	test('opens the task view scoped to the list chosen in the home view', async () => {
		...
		const ralph = fake.commands.get('ralph')!;
		await ralph.handler('import TODO.md', fakeCtx.ctx);
		const reclassified = readBacklog();
		reclassified.updateTaskById(1, { category: 'dossier' });
		reclassified.updateTaskById(2, { category: 'dossier' });
		reclassified.updateTaskById(3, { category: 'auth' });
		reclassified.save(autoFile());
		await ralph.handler('start --category dossier', fakeCtx.ctx);
	...
	test('shows the lists with counts; enter opens the task view for each list', async () => {
		...
		expect(homeText).toContain('Ralph home — test-session.db');
		...
		expect(component.render(100).join('\n')).toContain('Ralph backlog — test-session.db');
	});
	...
	test('falls back to the session ralph file; errors without TUI', async () => {
		const fake = createFakePi();
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		await fake.fire('session_start', fakeCtx.ctx, { reason: 'startup' });
		...
		// After the import, bare /ralph opens the session backlog.
		await ralph.handler('import TODO.md', fakeCtx.ctx);
		fakeCtx.customFactories.length = 0;
	...
	test('edits from the task view are written back to the backlog file', async () => {
		...
		await flush();
		expect(readBacklog().listTasks().map((t) => t.title)).toContain('New task from the view');
	});
	...
	test('the home view refreshes after a task-view round renames a list', async () => {
		...
		const handler = ralph.handler('', fakeCtx.ctx);
	...
	test('bare /ralph follows a persisted pre-migration .ralph state path to its .db sibling', async () => {
		const fake = createFakePi();
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		// The session's backlog exists only as the migrated SQLite database;
		// the persisted loop state (from before the SQLite migration) still
		// records the legacy .ralph name.
		Backlog.parse(RALPH_V1).save(autoFile());
		...
		await ralph.handler('', fakeCtx.ctx);
		expect(
			fakeCtx.notifications.some((n) => n.message.startsWith('No backlog found'))
		).toBe(false);
		expect(fakeCtx.customFactories.length).toBe(1);
		expect(
			(fakeCtx.customFactories[0]!({ requestRender: () => {} }, fakeTheme, undefined, () => {}) as { render: (w: number) => string[] }).render(100).join('\n')
...
describe('ralph-loop extension (set-goal command)', () => {
	...
	test('sets the goal from a file; a leading H1 marker is stripped', async () => {
		const fake = createFakePi();
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		...
		expect(backlog.goal()?.body).toBe('Port the complete app\n\nPort the app from the source to the target.\n\n- Criterion one holds.\n- Criterion two holds.');
		...
		expect(notification).toContain('Set the goal in');
		expect(notification).toContain('Start the goal loop with: /ralph start --goal');
	});

	test('the whole file is the goal without an H1', async () => {
		const fake = createFakePi();
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
	...
	test('replaces an existing open goal', async () => {
		...
		const backlog = readBacklog();
		expect(backlog.goal()?.body).toBe('Second goal\nSecond body.');
		expect(fakeCtx.notifications.at(-1)?.message).toContain('Replaced the goal in');
	});
...
T 1 - "Task one"
`;
		await writeFile(autoFile(), claimed);

		await setGoal(fake, fakeCtx, 'set-goal GOAL.md');

		expect(fakeCtx.notifications.at(-1)?.message).toContain('is claimed — resolve it first');
		// The refusal leaves the backlog content untouched (the file itself may
		// have been migrated to the SQLite form on open).
		expect(renderFile()).toBe(Backlog.parse(claimed).render());
	});
...
`;
		...
		// The refusal leaves the backlog content untouched (the file itself may
		// have been migrated to the SQLite form on open).
		expect(renderFile()).toBe(Backlog.parse(done).render());
	});

	test('targets the active loop’s backlog', async () => {
		const fake = createFakePi();
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		await fake.fire('session_start', fakeCtx.ctx, { reason: 'startup' });
	...
	test('refuses a missing goal file', async () => {
		...
		expect(fakeCtx.notifications.at(-1)?.message).toContain('Could not read MISSING.md');
		expect(readBacklog().goal()).toBeUndefined();
	});
	...
	test('refuses a non-ralph backlog target', async () => {
		...
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		await writeFile(autoFile(), '# not ralph\n');

		await setGoal(fake, fakeCtx, 'set-goal GOAL.md');

		expect(fakeCtx.notifications.at(-1)?.message).toContain('is not a ralph-format backlog');
	});
	...
	test('refuses a missing backlog target', async () => {
		...
		expect(fakeCtx.notifications.at(-1)?.message).toContain('No backlog at');
	});
	...
	test('refuses paths outside the project', async () => {
		...
		expect(fakeCtx.notifications.at(-1)?.message).toBe('Usage: /ralph set-goal <goal-file>');
	});
...
describe('ralph-loop extension (auto mode)', () => {
	...
	const autoTool = (fake: ReturnType<typeof createFakePi>) => {
		...
		// The auto loop's backlog is the per-session ralph file, which the tool
		// targets by default when no loop is active.
		return tool;
	};
	...
	beforeEach(async () => {
		// The auto loop starts from an empty session file (the project backlog
		// no longer exists); drop the outer fixture.
		await rm(autoFile(), { force: true });
	});
...
`
	...
	test('auto mode start names the session category after the named session', async () => {
		...
		expect(readBacklog().createdLists()).toEqual(['Fix-login-flow']);
	...
	test('auto mode refuses a custom category', async () => {
		await writeAutoConfig();
		const fake = createFakePi();
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		...
		// --todo no longer exists: it is a usage error.
		await ralph.handler('start --todo OTHER.ralph', fakeCtx.ctx);
		expect(fakeCtx.notifications.at(-1)?.message).toContain('Usage: /ralph start');
		await ralph.handler('start --category Email', fakeCtx.ctx);
		expect(fakeCtx.notifications.at(-1)?.message).toContain('Auto mode manages its own session category');
	...
	test('auto mode finish-up allows a bad state and records findings, with no goal layer', async () => {
		await writeAutoConfig();
		const fake = createFakePi();
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		...
		// Iteration 1 finishes up at the budget: a bad state is OK.
		fakeCtx.usagePercent.value = 55;
		await fake.fire('agent_settled', fakeCtx.ctx);
		...
		// Iteration 2 finishes up at the budget: the handoff still records
		// findings and has no goal layer.
		fakeCtx.usagePercent.value = 55;
		await fake.fire('agent_settled', fakeCtx.ctx);
	...
	test('ralph_todo add records a todo in the session category; complete marks it done with a log entry', async () => {
		...
		let file = readBacklog();
		expect(file.listTasks().map((t) => t.title)).toContain('Continue the rewrite');
		...
		file = readBacklog();
		expect(file.listTasks()[0]?.done).toBe(true);
		expect(file.listLogEntriesForTask(1).map((e) => e.note)).toEqual(['rewrote the parser']);
	});
	...
	test('ralph_todo add with a category creates it and records there; the default stays the session category', async () => {
		...
		const file = readBacklog();
		expect(file.listTasks()[0]?.category).toBe('Research');
		expect(file.listTasks()[1]?.category).toBe('General');
	...
	test('ralph_todo update changes a todo title and body instead of adding a duplicate', async () => {
		...
		let file = readBacklog();
		expect(file.listTasks().map((t) => t.title)).toContain('Finish the parser rewrite');
		expect(file.listTasks()[0]?.body).toContain('- add tests');
		// The old title is gone: no duplicate was created.
		expect(file.listTasks().map((t) => t.title)).not.toContain('Continue the rewrite');
		...
		file = readBacklog();
		expect(file.listTasks()[0]?.body).toBeNull();
	...
	test('ralph_todo add starts the auto loop when auto mode is on and no loop is active', async () => {
		...
		expect(readBacklog().listTasks().map((t) => t.title)).toContain('First step');
	...
	test('concurrent ralph_todo adds arm the auto loop once and both todos survive (no torn file)', async () => {
		...
		// load → mutate → write cycles: no lost todo, no torn file. Which
		// sibling gets task number 1 is a race (the first to reach the file
		// lock wins); exactly one reports the arming.
		const results = [first.content[0]!.text, second.content[0]!.text];
		expect(results.join('\n')).toContain('Added task 1');
		expect(results.join('\n')).toContain('Added task 2');
		expect(results.filter((text) => text.includes('The Ralph auto loop was started'))).toHaveLength(1);
		const armed = stateEntries(fake).filter((entry) => (entry.data as { enabled?: boolean }).enabled === true);
		expect(armed).toHaveLength(1);

		// The file still opens and both todos are in the session category.
	...
	test('auto mode "on" mid-turn: crossing the budget in a plain session steers the finish-up', async () => {
		...
		expect(files).toEqual(['test-session.db']);
		expect(readBacklog().createdLists()).toEqual(['General']);
		expect(statusLine(fakeCtx.widgets)).toContain('iteration 1/10');
	});
	...
	test('ralph_goal checkpoint is refused in the auto loop (the auto loop has no goal layer)', async () => {
		await writeAutoConfig();
		await writeFile(
			autoFile(),
			'# ralph v2\n\nG "Working desktop app" open\nGB\n  - verified analysis\n'
		);
		const fake = createFakePi();
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		...
		await expect(
			goalTool(fake).execute('t', { action: 'checkpoint', note: 'x' }, undefined, undefined, fakeCtx.ctx)
		).rejects.toThrow(/active goal loop/);
	});

	test('auto iteration prompt carries no goal section (the auto loop has no goal layer)', async () => {
		await writeAutoConfig();
		await writeFile(
			autoFile(),
			'# ralph v2\n\nG "Working desktop app" open\nGB\n  - verified analysis\n\nT 1 - "A task."\n'
		...
		const prompt = fake.userMessages[0]!.text;
		expect(prompt).toContain('Run the Ralph auto loop');
		expect(prompt).not.toContain('Big picture');
		expect(prompt).not.toContain('ralph_goal');
	});
});
...
`;
	...
	// The auto loop's backlog is the per-session ralph file, which the tool
	// targets by default when no loop is active.
	const autoTool = (fake: ReturnType<typeof createFakePi>) => todoTool(fake);
...
`;
	...
	test('D1: update rewrites a planned task in the task loop (title and body)', async () => {
		...
		const file = readBacklog();
		expect(file.listTasks().map((t) => t.title)).toContain('Task one (revised)');
		expect(file.listTasks()[0]?.body).toContain('- replanned');
		// The old title is gone: no duplicate was created.
		expect(file.listTasks().map((t) => t.title)).not.toContain('Task one');
	});
	...
	test('D2: add-many records a batch in the auto loop', async () => {
		...
		const titles = readBacklog().listTasks().map((t) => t.title);
		expect(titles).toContain('First step');
		expect(titles).toContain('Second step');
	});

	test('D3: move reorders tasks in the auto loop', async () => {
		await writeAutoConfig();
		// The auto loop starts from an empty session file (the outer fixture
		// is a task-loop backlog).
		await rm(autoFile(), { force: true });
		const fake = createFakePi();
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		...
		expect(readBacklog().listTasks().map((t) => t.title)).toEqual(['Second step', 'First step']);
	});

	test('D4: next skips Findings reference entries on the session backlog', async () => {
		await writeFile(
			autoFile(),
			'# ralph v2\n\nT 1 - "Findings: root cause was the cache"\n\nT 2 - "Real work"\n'
		);
		const fake = createFakePi();
		extension(fake.pi as never);
		const fakeCtx = createFakeCtx(dir);
		...
		const tool = todoTool(fake);
		const next = await tool.execute('t', { action: 'next' }, undefined, undefined, fakeCtx.ctx);
		// Reference entries ("Findings: ") are not work items.
		expect(next.content[0]!.text).toContain('Real work');
		expect(next.content[0]!.text).not.toContain('Findings:');
	});

	test('D5: idle reads target the session ralph file', async () => {
		await writeFile(
			autoFile(),
