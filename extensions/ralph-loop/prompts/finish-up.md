...
3. Record the remaining work: {{#if checkpointHandoff}}for the in-progress task, record its progress with ralph_todo (action "checkpoint", the task number, a concise note) that is self-contained about where the task stands; use action "update" only if the task's title or body itself is stale or wrong, and action "add" (title, optional body{{categoryClause}}) only for genuinely new work items.{{else}}for the in-progress task, update it with ralph_todo (action "update") so its body is self-contained about what remains; use action "add" (title, optional body{{categoryClause}}) only for genuinely new work items.{{/if}} A fresh session has none of this conversation, so {{#if checkpointHandoff}}the checkpoint note and each new entry include:{{else}}each entry includes:{{/if}}
   - {{#if checkpointHandoff}}what is done and what remains,{{else}}what remains{{/if}} and why
   - relevant paths
