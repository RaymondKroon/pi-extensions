Run the Ralph auto loop for this repository. {{contextNote}}{{goalSection}}
...
2. If there is an open task: read the relevant code and source evidence, then implement exactly one coherent vertical slice. Add focused tests and run every quality command required by the backlog and {{specPath}} (when present).
