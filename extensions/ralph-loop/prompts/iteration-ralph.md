...
1. Call ralph_todo with action "next" to get the next open task{{categoryScope}}: its number, body, and checkpoint. Use action "list" only when that task is blocked and you need the wider backlog to find an unblocked one.
...
4. Add focused tests and run every quality command required by the backlog.
