...
/**
 * Parse a generation request. When neither output option is supplied, generate
 * both conventional files; when one or both are supplied, generate exactly
 * those named files. `--` permits a brief that starts with an option-looking
 * word.
 */
function parseInitFiles(args: string[]): RalphInitFiles | undefined {
	...
	return {
		specFile: specFile ?? (todoFile ? undefined : DEFAULT_SPEC),
		todoFile: todoFile ?? (specFile ? undefined : DEFAULT_TODO),
		force,
		prompt
	};
}
...
function initPrompt(files: RalphInitFiles): string {
	const targets = [
		...(files.specFile ? [`specification: ${files.specFile}`] : []),
		...(files.todoFile ? [`backlog: ${files.todoFile}`] : [])
	].join(', ');
	const templateWarning = [files.specFile, files.todoFile].filter(
		(file) => file === DEFAULT_SPEC || file === DEFAULT_TODO
	);
	return `Create the requested Ralph planning document${files.specFile && files.todoFile ? 's' : ''} now. This is planning work only; do not implement the product brief.
...
First read the bundled generic planning templates in full:
- specification template: ${INIT_TEMPLATE_SPEC}
- TODO template: ${INIT_TEMPLATE_TODO}

They are the authoritative examples for the level of product/engineering detail, durable-spec versus ordered-backlog split, Ralph protocol, acceptance criteria, decision handling, source evidence, and completion-log conventions. Adapt their structure and rigor to this project brief; do not copy their placeholder text or assume the project has an existing ${DEFAULT_SPEC} or ${DEFAULT_TODO}.

Create exactly the target file${files.specFile && files.todoFile ? 's' : ''} above${files.force ? ', replacing the named existing file(s) because --force was explicitly supplied,' : ''}. Do not modify any other file${templateWarning.length > 0 ? `; the target ${templateWarning.join(' and ')} is intentionally allowed because it was explicitly selected` : ''}. Use the write tool to produce complete Markdown documents, not a prose preview. If both are requested, make their cross-links and Ralph instructions reference the selected filenames. If only one is requested, make it self-contained while linking to the corresponding selected/default counterpart where useful.

The specification must be a durable, implementation-ready product and engineering contract: purpose, scope, non-goals, source/evidence rules where applicable, architecture, domain/lifecycle and authorization constraints, user journeys and acceptance criteria, quality/security requirements, definition of done, and release gates. The TODO must be a prioritized, executable Ralph backlog of small vertical slices, with a read-first Ralph protocol, concrete acceptance criteria, source/decision evidence expectations, deferred/out-of-scope work, and an empty completion-log scaffold. Derive scope, architecture, risks, quality checks, and decisions from the project brief; identify unknowns explicitly rather than inventing them. After writing, read each generated file and verify that it is complete, internally consistent, and contains no unrelated implementation changes. Then report the generated paths succinctly.`;
}
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerCommand('ralph-init', {
		description: 'Create Ralph SPEC/TODO planning docs: [--spec file] [--todo file] [--force] <project brief>',
		getArgumentCompletions: (prefix): AutocompleteItem[] | null => {
			const options: AutocompleteItem[] = [
				...
				{ value: '--todo', label: '--todo', description: 'Generate only this backlog file (or pair with --spec).' },
		...
		handler: async (args, ctx) => {
			...
			const requestedFiles = [initFiles.specFile, initFiles.todoFile].filter(
			...
			const paths = requestedFiles.map((file) => ({ file, path: resolveProjectFile(ctx.cwd, file) }));
			...
			ctx.ui.notify(`Preparing Ralph planning document${requestedFiles.length === 1 ? '' : 's'}…`, 'info');
			pi.sendUserMessage(initPrompt(initFiles));
		}
	});
